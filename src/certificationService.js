...
module.exports = class CertificationService {
     ...
     */
    constructor({appDir, proxyDataDir}) {
        this.appDir = appDir;
        // 监控数据缓存目录
    ...
    async start() {
        let key = await fileUtils.readFile(path.join(this.appDir, 'certificate/zproxy.key.pem'));
        let cert = await fileUtils.readFile(path.join(this.appDir, 'certificate/zproxy.crt.pem'));
        this.root = {
            cert,
            key
        };
